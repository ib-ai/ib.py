...
class ContentModal(discord.ui.Modal, title="Edit Message Content"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        args = parse_json_or_text(self.content.value)
...
class BotMessages(commands.Cog):
    ...
    async def interaction_ensure_staff(self, interaction: discord.Interaction) -> bool:
        if not staff_command().predicate(interaction):
    ...
    async def ctx_menu_get_json(
        ...
        if len(output) > 2000:
            ...
            file = discord.File(
                fp=io.StringIO(json.dumps(result, indent=None)), filename="message.json"
            )
            await interaction.response.send_message(
    ...
    async def ctx_menu_edit_message(
        ...
        """
        if not await self.interaction_ensure_staff(interaction):
            return
        await interaction.response.send_modal(
            ContentModal(message=message, action=lambda m, **args: m.edit(**args))
        )
    ...
    async def send(
        ...
        *,
        ...
        if not json_or_text:
            await ctx.send(
                f"Please provide message content or JSON input. Use [{EMBED_GUI_URL}]({EMBED_GUI_URL}) to construct embeds."
            )
            return

        args = parse_json_or_text(json_or_text)
    ...
    async def embedraw(
        ...
        discord_json = json.dumps(payload, indent=2)
        warning_msg = "⚠️ This command is deprecated. Please use the /send or /edit commands with the new JSON input instead. (To get a message's raw JSON, right click -> Apps -> Get Message JSON.) ⚠️"
        output_str = f"```json\n{discord_json}\n```"
