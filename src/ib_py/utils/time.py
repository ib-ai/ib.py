...
DEGENERACY_DELAY = timedelta(
    seconds=1
)  # when datetime is in the past, a small amount of time is slept for regardless
...
def parse_time(s: str) -> timedelta:
    ...
    for value, key in re.findall("(\d+)(\D+)", s):
        ...
        if key.startswith("y"):
            ...
            if (
                now.month == 2 and now.day == 29
            ):  # for when you add 1 year to the 29th of February
