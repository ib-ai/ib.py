...
class Public(commands.Cog):
    ...
    async def ping(self, ctx: commands.Context):
        ...
        """
        embed = discord.Embed(
            description=f"I don't see how this will help you, but my ping is `{round(self.bot.latency * 1000)}ms`.",
    ...
    async def serverinfo(self, ctx: commands.Context):
        ...
        embed.add_field(
            name="**Creation Date**", value=f"{timestamp} ({relative})", inline=False
        )
        ...
        embed.add_field(
            ...
            value=f"{str(ctx.guild.verification_level).capitalize()}",
        )
        ...
        embed.add_field(
            name="**Channels**",
            value=f"{txt} text, {vc} voice, {ctg} categories, {stg} stage, {frm} forum. {total} total.",
            inline=False,
        )
